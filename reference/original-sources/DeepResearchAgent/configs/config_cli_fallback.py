"""
CLI-First Configuration with Local Fallbacks

This configuration prioritizes CLI tools but falls back to local models
when CLI tools are not available, avoiding the KeyError issue.
"""

_base_ = './base_cli_fallback.py'
...
# Planning agent - uses Claude CLI or fallback to local
planning_agent_config = dict(
    type="planning_agent",
    name="planning_agent",
    model_id="claude-3.7-sonnet-thinking",  # Will map to CLI or fallback
...
# Deep researcher - uses Claude CLI or local fallback
...
# Deep analyzer - uses Claude CLI or local fallback
...
# Browser use - uses local models (GPT-4 not critical for this demo)
browser_use_agent_config = dict(
    type="browser_use_agent",
    name="browser_use_agent",
    model_id="qwen2.5-32b-instruct",  # Use local model for browser tasks
